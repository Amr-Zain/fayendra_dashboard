...
const AppUploader = <T extends object = Record<string, any>>({
 ...
 // name = "image",
  ...
  const handlePreview = async (file: UploadFile) => {
    ...
    if (type_file === "document" || file.type?.includes('pdf')) {
      setPreviewType("document");
      setPreviewContent(file.url || (file.preview as string));
    } else if (type_file === "media" || file.type?.startsWith("video")) {
  ...
  const handleFileUpload = async (files: FileType[]) => {
    try {
      ...
      for (const file of newFiles) {
        try {
          ...
          const updatedFile: UploadFile = {
            ...
            isUploading: false,
            response: response,
            url: response.data.url || file.url, // Use server URL if available, fallback to blob URL
          };
      ...
      if (uploadedFiles.length > 0) {
        if (onChange) {
          if (singleFile) {
            onChange(uploadedFiles[0]); // Single file: pass the file data directly
          } else {
            onChange(uploadedFiles); // Multiple files: pass array of file data
          }
        } else if (field) {
  ...
  const handleRemove = async (file: UploadFile) => {
    try {
      ...
      if (file.response?.data?.id || (file.uid && !file.url?.startsWith("blob:"))) {
        ...
        try {
          await axiosInstance.delete(`${apiEndpoint}/${fileId}`);
        } catch (deleteError) {
